...
const ItemTool = ({ tool }) => {
    ...
    const statusColor = status_tool === "Habilitado" ? "text-green-600" : "text-red-600"
        ...
        <div onClick={() => handleToolClick(id_tool)} className="flex w-full max-w-[457.5px] h-[137.5px] my-4 rounded-lg overflow-hidden shadow-lg transition-all duration-150 ease-in-out hover:shadow-md cursor-pointer">
            ...
            <div className="flex-3 p-2">
                <div className="flex flex-col h-full">
                    ...
                    <p className="text-gray-600 text-sm font-semibold">Ubicación: {location_tool}</p>
                    <p className={`text-lg font-semibold ${statusColor}`}>Estado: {status_tool}</p>
                </div>
            </div>
        </div>
    )
