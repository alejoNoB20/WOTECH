...
const UpdateStock = () => {
  ...
  const handleUpdate = async (e) => {
    try {
      e.preventDefault();
      const { products, suppliers, id_material, ...materialData } =
        updatedMaterial;
  ...
  const handleDisabled = async () => {
    const confirmDelete = window.confirm(
      `Estás seguro que querés eliminar el material "${material.name_material}"`
    );
    if (confirmDelete) {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/stock/disabled/${id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      {!showForm ? (
        ...
        >
          ...
          <form onSubmit={handleUpdate} className="flex flex-col mb:mx-auto">
            ...
            <div className="mb-4">
              {/* NOMBRE */}
              <label htmlFor="name_material" className="block text-gray-700">
                Nombre: *
              </label>
              <input
                type="text"
            ...
            <div className="mb-2">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setUpdatedMaterial({
                    ...updatedMaterial,
                    description_material: e.target.value,
                  })
                }
                className="mt-1 w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              />
              ...
              >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setUpdatedMaterial({
                      ...updatedMaterial,
                      measurement_material: e.target.value,
                    })
                  }
            ...
            {/* CANTIDAD */}
            <div className="mb-4">
              <label htmlFor="amount_material" className="block text-gray-700">
                Cantidad: *
              </label>
              <input
                type="number"
                ...
                onChange={(e) =>
                  setUpdatedMaterial({
                    ...updatedMaterial,
                    amount_material: e.target.value,
                  })
                }
                className="mt-1 w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                required
