...
const DetailsSupplier = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/suppliers/details/${id}`,
          {
            method: "GET",
            credentials: "include",
          }
  ...
  const handleCUITorCUIT = (e) => {
    ...
    if (e.target.value.length < 11) {
      if (regExNum.test(e.nativeEvent.key)) {
        setUpdatedSupplier({
          ...updatedSupplier,
          cuit_company_supplier: preValue + e.nativeEvent.key,
        });
      }
    }
    ...
    if (e.nativeEvent.key === "Backspace") {
      if (e.target.value.length === 1) {
        ...
        setUpdatedSupplier({
          ...updatedSupplier,
          cuit_company_supplier: preValue.slice(0, -1),
        });
      }
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    const responseMaterials = await fetch(
    ...
    if (updatedMaterials.length > 0) {
      ...
      const supplierMaterials = updatedMaterials.map((material) => {
        return {
          ...
          id_supplier_fk: material.supplierStockAssociations.id_supplier_fk,
          amount_material: material.supplierStockAssociations.amount_material,
          price_material: material.supplierStockAssociations.price_material,
          disabled: material.supplierStockAssociations.disabled,
      ...
      for (const material of supplierMaterials) {
        if (!material.disabled) {
          const usedMaterialIndex = responseMaterialsJSON.findIndex(
            (usedMaterial) =>
              usedMaterial.id_material === material.id_material_fk
          );
      ...
      setUpdatedSupplier({
        ...supplier,
        stocks: "Este proveedor aún no tiene materiales asociados",
      });
    }
  ...
  const handleAddMaterial = () => {
    if (
      materialRef &&
      priceMaterialRef.length !== 0 &&
      priceMaterialRef > 0 &&
      amountMaterialRef.length !== 0 &&
      amountMaterialRef > 0
    ) {
      ...
      if (typeof materialSelected !== "string") {
        ...
        stocks.push({
          id_material_fk: materialRef.id_material,
          id_supplier_fk: supplier.id_supplier,
          amount_material: Math.round(amountMaterialRef),
          price_material: Math.round(priceMaterialRef),
          disabled: false,
          name_material: materialRef.name_material,
        });
  ...
  const handleRemoveMaterial = (material) => {
    ...
    const removeMaterial = materialSelectedList.filter(
      (materialSelected) =>
        materialSelected.id_material_fk !== material.id_material_fk
    );
  ...
  const handleDisable = async () => {
    try {
      ...
      if (confirmDelete) {
        setLoader(true);
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/suppliers/disabled/${id}`,
          {
            method: "PATCH",
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/suppliers/update/${id}`,
        {
          method: "PATCH",
      ...
      if (typeof stocks !== "string") {
        if (!hasMaterials) {
          ...
          materialResponse = await fetch(
            `${process.env.REACT_APP_API_URL}/suppliers/supplierMaterials/update/${id}`,
            {
              method: "PATCH",
          ...
          materialResponse = await fetch(
            `${process.env.REACT_APP_API_URL}/suppliers/supplierMaterials/create`,
            {
              method: "POST",
              body: JSON.stringify(stocks),
              headers: {
        ...
        >
          ...
          <section className="flex flex-col bg-white rounded-xl shadow-xl mx-3 mb:my-4 mb:px-5 mb:py-5 md:py-5 md:my-3 md:px-20 justify-center items-center mb:text-center">
            ...
            <h2 className="text-2xl font-bold mb-4 text-center">
              Detalles del proveedor: {supplier.name_company_supplier}{" "}
              {supplier.reason_social_supplier &&
                `(${supplier.reason_social_supplier})`}
            </h2>
            {/* COLUMNA DE INPUTS */}
            <div className="flex flex-col md:flex-row mx-3">
              ...
              <div className="flex flex-col md:p-2 md:w-full">
                ...
                <div className="mb-4">
                  ...
                  <p className="text-gray-800 ">
                    {supplier.number_phone_distributor_supplier ||
                      "No disponible"}
                  </p>
                </div>
            ...
            <div className="flex flex-col mb-4 w-auto md:mx-40">
              <h4 className="text-gray-400 text-xs font-semibold">
                Lista de materiales que vende {supplier.name_company_supplier}:
              </h4>
              <select
                name="priceControl"
                id="priceControl"
                    ...
                    <>
                      {!material.supplierStockAssociations.disabled && (
                        ...
                        >
                          ...
                          {`${material.supplierStockAssociations.price_material}`}{" "}
                          {`(${material.supplierStockAssociations.amount_material})`}
                        </option>
                      )}
                    </>
                  ))}
        ...
        >
          ...
          <section className="flex flex-col bg-white rounded-xl shadow-xl mx-3 mb:my-9 mb:px-2 mb:py-5 md:py-5 md:my-3 md:px-20">
            ...
            <h2 className="text-2xl font-bold mb-4 mx-5">
              Actualizar Proveedor: {updatedSupplier.name_company_supplier}
              {updatedSupplier.reason_social_supplier &&
                `(${updatedSupplier.reason_social_supplier})`}
            </h2>
            {/* COLUMNA DE INPUTS */}
            <div className="flex flex-col md:flex-row">
              ...
              <div className="flex flex-col md:p-2 mx-5">
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        name_company_supplier: e.target.value,
                      })
                    }
                    required
                  />
                ...
                <div className="mb-4">
                  <label
                    htmlFor="reason_social_supplier"
                    className="block text-gray-700"
                  >
                    Razón social:
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        reason_social_supplier: e.target.value,
                      })
                    }
                  />
                </div>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        tax_address_supplier: e.target.value,
                      })
                    }
                    required
                  />
              ...
              <div className="flex flex-col md:p-2 mx-5">
                ...
                <div className="mb-4">
                  <label
                    htmlFor="number_phone_company_supplier"
                    className="block text-gray-700"
                  >
                    Número de teléfono: *
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        number_phone_company_supplier: e.target.value,
                      })
                    }
                    required
                  />
                ...
                <div className="mb-4">
                  <label
                    htmlFor="website_company_supplier"
                    className="block text-gray-700"
                  >
                    Sitio Web:
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        website_company_supplier: e.target.value,
                      })
                    }
                  />
                </div>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        mail_company_supplier: e.target.value,
                      })
                    }
                  />
                </div>
                ...
                <div className="mb-4">
                  <label
                    htmlFor="distributor_name_supplier"
                    className="block text-gray-700"
                  >
                    Nombre del distribuidor:
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        distributor_name_supplier: e.target.value,
                      })
                    }
                  />
                </div>
              ...
              <div className="flex flex-col md:p-2 mx-5">
                ...
                <div className="mb-4">
                  <label
                    htmlFor="number_phone_distributor_supplier"
                    className="block text-gray-700"
                  >
                    Número del distribuidor:
                  </label>
                  <input
                    type="text"
                    value={
                      updatedSupplier.number_phone_distributor_supplier || ""
                    }
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        number_phone_distributor_supplier: e.target.value,
                      })
                    }
                  />
                </div>
                ...
                <div className="mb-4">
                  <label
                    htmlFor="mail_distributor_supplier"
                    className="block text-gray-700"
                  >
                    Email del distribuidor:
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        mail_distributor_supplier: e.target.value,
                      })
                    }
                  />
                </div>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        delivery_days_suppier: e.target.value,
                      })
                    }
                  />
                </div>
                ...
                <div className="mb-4">
                  <label
                    htmlFor="payment_method_supplier"
                    className="block text-gray-700"
                  >
                    Método de pago:
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      setUpdatedSupplier({
                        ...updatedSupplier,
                        payment_method_supplier: e.target.value,
                      })
                    }
                  >
            ...
            <div className="mb-4 mx-7">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setUpdatedSupplier({
                    ...updatedSupplier,
                    description_supplier: e.target.value,
                  })
                }
              />
              <CharacterCounter value={updatedSupplier.description_supplier} />
            </div>
            ...
            <div className="flex flex-col md:flex-row w-full mb-3 gap-2 md:gap-5 md:justify-center md:items-center">
              ...
              {/* CANTIDADES */}
              <div className="flex flex-col">
                <label htmlFor="amountMaterial" className="block text-gray-700">
                  Cantidades:
                </label>
                <input
                  type="number"
            ...
            {typeof updatedSupplier.stocks === "string" ? (
              <div className="flex flex-row px-5">
                <h3 className="text-gray-700">{updatedSupplier.stocks}</h3>
              </div>
            ) : (
              <>
                {updatedSupplier.stocks?.length > 0 &&
                  updatedSupplier.stocks.map((material) => (
                    <>
                      {!material.disabled && (
                        <div
                          key={material.id_material_fk}
                          ...
                          <button
                            ...
                            onClick={() =>
                              handleRemoveMaterial(
                                updatedSupplier.stocks.find(
                                  (mat) =>
                                    mat.id_material_fk ===
                                    material.id_material_fk
                                )
                              )
                            }
                          >
            ...
            <section className="flex flex-row my-4">
              <button
                className="mx-auto bg-green-700 hover:bg-green-900 text-white font-bold py-2 px-4 rounded-lg"
              ...
              <button
                className="mx-auto bg-indigo-600 hover:bg-indigo-800 text-white font-bold py-2 px-4 rounded-lg"
