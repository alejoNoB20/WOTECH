...
const PurchaseMaterials = () => {
  ...
  const handleSupplierList = async (e) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/suppliers/details/${e.target.value}`,
        {
          method: "GET",
          credentials: "include",
        }
  ...
  const handleAddMaterial = () => {
    if (selectRef !== "none" && selectRef && amountRef && amountRef > 0) {
      ...
      finalMaterialList.push({
        id_material: selectRef.id_material,
        name_material: selectRef.name_material,
        price_material: selectRef.supplierStockAssociations.price_material,
        amount_material: selectRef.supplierStockAssociations.amount_material,
        id_supplier_material:
          selectRef.supplierStockAssociations.id_supplier_material,
        unit_material: amountRef,
      });
  ...
  const handleRemoveMaterial = async (id_material) => {
    ...
    const finalMaterialList = finalPurchase.purchase.filter(
  ...
  const handleBuy = async () => {
    try {
      ...
      const finalList = finalPurchase.purchase.map((material) => {
        return {
          id_supplier_material: material.id_supplier_material,
      ...
      console.log(err);
      handleFail(
        "Ops! ah ocurrido un error inesperado a la hora de generar la compra"
      );
      setLoader(false);
    }
  };
      ...
      {loader ? (
        ...
        <section className="flex w-full h-full justify-center bg-gray-200">
          ...
          <section className="flex flex-col bg-white rounded-xl shadow-xl mx-3 mb:my-9 mb:px-2 mb:py-5 md:py-5 md:my-3 md:px-20">
            ...
            <div className="mb-6">
              <label htmlFor="supplierList" className="block text-gray-500">
                Lista de proveedores: *
              </label>
              <select
                name="supplierList"
            ...
            {supplier && (
              <div className="flex flex-row justify-center gap-2 mx-2 mb-4">
                ...
                <div className="flex flex-col">
                  <label htmlFor="supplier" className="block text-gray-500">
                    Materiales: *
                  </label>
                  <select
                    name="supplier"
                    ...
                    onChange={(e) =>
                      setSelectRef(
                        supplier.stocks.find(
                          (material) =>
                            material.id_material === Number(e.target.value)
                        )
                      )
                    }
            ...
            {finalPurchase.purchase?.length > 0 &&
              finalPurchase.purchase.map((material) => (
                <div key={material.id_material} className="flex flex-row mb-3">
                  <div className="flex flex-row px-5">
                    <h3 className="text-gray-700">
                      - {material.name_material}: $
                      {`${material.price_material}`}{" "}
                      {`(${material.amount_material} x ${material.unit_material})`}{" "}
                      | TOTAL: (
                      {material.amount_material * material.unit_material})
                    </h3>
                  </div>
                  <button
                    className="bg-red-700 text-white rounded-lg border border-gray-300 px-2 hover:bg-red-800"
                    onClick={() => handleRemoveMaterial(material.id_material)}
                  >
                    Eliminar
                  </button>
                </div>
