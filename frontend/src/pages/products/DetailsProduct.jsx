...
const DetailsProduct = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/products/details/${id}`
        );
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/products/getStockAndTools`
      );
      const responseJSON = await response.json();
      ...
      for (const material of updateProduct.stocks) {
        const usedMaterialIndex = responseJSON.stock.findIndex(
          (usedMaterial) => usedMaterial.id_material === material.id_material
        );
        responseJSON.stock[usedMaterialIndex].disabled = true;
        listMaterialSelected.push({
          id: material.id_material,
          name_material: material.name_material,
          how_much_content: Math.round(
      ...
      for (const tool of updateProduct.tools) {
        const usedToolIndex = responseJSON.tools.findIndex(
          (usedTaterial) => usedTaterial.id_tool === tool.id_tool
        );
  ...
  const handleAddMaterial = () => {
    if (
      ...
      materials.push({
        id: materialRef.id_material,
        name_material: materialRef.name_material,
        how_much_content: Math.round(quantityMaterialRef),
      });
  ...
  const handleRemoveMaterial = (material) => {
    ...
    const removeMaterial = materialSelectedList.filter(
      (materialSelected) => materialSelected.id !== material.id
    );
  ...
  const handleRemoveTool = (eliminatedTool) => {
    ...
    const removeTool = toolSelectedList.filter(
      (toolSelected) => toolSelected.id_tool !== eliminatedTool.id_tool
    );
  ...
  const handleDisable = async (e) => {
    try {
      ...
      if (confirmDelete) {
        setLoader(true);
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/products/disabled/${id}`,
          {
            method: "PATCH",
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoader(true);
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/products/update/${updateProduct.id_product}`,
        {
          method: "PATCH",
          body: formData,
          credentials: "include",
      ...
      console.log(err);
      handleFail(
        "Ups!, un error ocurrio a la hora de manejar la información, inténtalo nuevamente"
      );
      setLoader(false);
    }
  };
            ...
            >
              ...
              <div className="flex flex-col mb-4 w-full">
                ...
                <select
                  ...
                  className="mt-1 mx-4 px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 mb:text-xs"
                >
                  <option value="none">
              ...
              <div className="flex flex-col mb-4 w-full">
                ...
                <select
                  ...
                  className="mt-1 block px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 mb:text-xs"
                >
                  <option value="none">
            ...
            >
              ...
              <div className="flex flex-col md:flex-row mx-3">
                ...
                <div className="flex flex-col w-auto min-w-[340px] px-10 justify-center text-center items-center">
                  <button
                    className="rounded-lg text-white bg-indigo-600 hover:bg-indigo-800 font-bold mx-auto mb-2 py-2 px-10"
                    onClick={() => {
                      !changeImg ? setChangeImg(true) : setChangeImg(false);
                    }}
                  >
                  ...
                  {changeImg ? (
                    <input
                      type="file"
                      className="mt-3 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm my-3"
                      id="img_product"
                      name="img_product"
                ...
                <div className="flex w-full flex-col space-y-4 mb-4 md:space-y-7 md:mx-10 items-center">
                  ...
                  <div className="mb-2">
                    ...
                    <div className="flex flex-row items-center">
                      <p className="text-lg text-gray-700 justify-center text-center mr-2">
                        $
                      </p>
                      <input
                        type="number"
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleRef(
                            ...
                            tools.find(
                              (tool) =>
                                tool.id_tool === parseInt(e.target.value)
                            )
                          )
                        }
                      >
              ...
              {/* Botonera */}
              <section className="flex flex-row my-4 gap-x-4 w-full mx-auto justify-center items-center">
                <button
                ...
                <button
                  className="bg-indigo-600 hover:bg-indigo-800 text-white font-bold py-2 px-6 rounded-lg"
