...
const DetailsOrders = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/orders/details/${id}`,
          {
            method: "GET",
            credentials: "include",
          }
        ...
        const daysDiff = diffDays(
          responseJSON[0].delivery_day_order,
          new Date()
        );
        ...
        console.log(err);
        handleFail(
          "Ops, ocurrio un error al momento de recuperar los datos de este pedido"
        );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      for (const product of order.products) {
        const productFilter = newList.findIndex((usedProduct) => {
          return usedProduct.id_product === product.id_product;
        });
      ...
      console.log(err);
      handleFail(
        "Ops, ocurrio un error al momento de querer ir a actualizar el pedido"
      );
      setModal(false);
      setLoader(false);
    } finally {
  ...
  const handleFinish = async () => {
    try {
      ...
      if (confirmDelete) {
        setLoader(true);
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/orders/disabled/${id}`,
          {
            method: "PATCH",
  ...
  const handleDelete = async () => {
    try {
      ...
      if (confirmDelete) {
        setLoader(true);
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/orders/delete/${id}`,
          {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
  ...
  const handleAddProduct = () => {
    if (productRef && UnitRef > 0) {
      ...
      orderList.push({
        id_product: productRef.id_product,
        name_product: productRef.name_product,
        price_product: productRef.price_product,
        orderProductsAssociation: { unit_product: UnitRef },
      });
  ...
  const handleRemoveMaterial = (id_product) => {
    ...
    const newProductSelectedList = currentOrder.filter(
    ...
    try {
      ...
      const finalProductList = updatedOrder.products.map((product) => {
        return {
          ...
          unit_product: product.orderProductsAssociation.unit_product,
        };
      });
      Body = { ...Body, products: finalProductList };

      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/orders/update/${id}`,
        {
          method: "PATCH",
            ...
            {!modal ? (
              ...
              <section className="flex flex-col bg-white rounded-xl shadow-xl mx-6 mb:my-6 mb:px-2 mb:py-5 md:py-5 md:my-3 md:px-20">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex flex-col mb-4">
                      ...
                      <p className={`${dayColorText} text-lg`}>
                        {format(order.delivery_day_order, "DD/MM/YYYY") ||
                          "No disponible"}
                      </p>
                    </div>
                  </div>
                    ...
                    {order.products?.length > 0 &&
                      order.products.map((product) => (
                        <option value={product.id}>
                          ...
                          {product.orderProductsAssociation.unit_product})
                        </option>
                      ))}
                  </select>
                </div>
                      ...
                      {materialUsed.length > 0 &&
                        materialUsed.map((material) => (
                          ...
                          >
                            ...
                            {material.how_much_contains_use})
                          </option>
                        ))}
                    </select>
                  </div>
                ...
                <section className="flex flex-row justify-center gap-2">
                  <button
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-500 font-bold"
              ...
              <section className="flex flex-col bg-white rounded-xl shadow-xl mx-6 mb:my-6 mb:px-2 mb:py-5 md:py-5 md:my-3 md:px-20">
                ...
                <div className="flex flex-col mb-4">
                  ...
                  >
                    ...
                    {clients.length > 0 &&
                      clients.map((client) => (
                        <option value={client.id_client} key={client.id_client}>
                ...
                {/* SECCION DE FECHA */}
                <div className="flex flex-col mb-4">
                  <label htmlFor="deliberyDay" className="text-gray-700">
                    Fecha de entrega: *
                  </label>
                  <input
                    type="date"
                    ...
                    onChange={(e) =>
                      setUpdatedOrder({
                        ...updatedOrder,
                        delivery_day_order: e.target.value,
                      })
                    }
                    required
                  />
                ...
                <div className="flex flex-col mb-4">
                  <label
                    htmlFor="shipping_address_order"
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUpdatedOrder({
                        ...updatedOrder,
                        shipping_address_order: e.target.value,
                      })
                    }
                  />
                </div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setProductRef(
                          productList.find(
                            (product) =>
                              product.id_product === Number(e.target.value)
                          )
                        )
                      }
                    >
                ...
                {Array.isArray(updatedOrder.products) &&
                  ...
                  updatedOrder.products.map((product) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => handleRemoveMaterial(product.id_product)}
                      >
                        Eliminar
                      </button>
                    </div>
                ...
                <section className="flex flex-row gap-3">
                  ...
                  <button
                    className="mx-auto bg-indigo-600 hover:bg-indigo-800 text-white font-bold py-2 px-6 rounded-lg"
