...
const UpdateTool = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/tools/details/${id}`
        );
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/tools/update/${id}`,
        {
          method: "PATCH",
  ...
  const handleDisable = async () => {
    ...
    if (confirmDelete) {
      setLoading(true);
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/tools/disabled/${id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
                ...
                {tool.status_tool === "En Arreglo" && (
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-gray-800 text-lg">
                        {format(tool.repair_date_tool, "DD/MM/YYYY") ||
                          "No disponible"}
                      </p>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-gray-800 text-lg">
                        {format(tool.search_repair_tool, "DD/MM/YYYY") ||
                          "No disponible"}
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex flex-col md:flex-row mx-3">
                  ...
                  <div className="flex flex-col w-auto min-w-[340px] px-10 justify-center text-center items-center">
                    ...
                    {changeImg ? (
                      <input
                        type="file"
                        className="mb-4 mb:w-30 md:w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 mb:text-xs"
                        id="img_product"
                        name="img_product"
                      ...
                      <img
                        id="img"
                        src={updateTool.img_tool}
                        alt={`Imagen de ${updateTool.name_tool}`}
                        className="rounded-lg mb-4 mb:max-w-40
