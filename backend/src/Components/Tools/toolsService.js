...
export class ToolsService {
    ...
    try {
      ...
      if (resultado.length === 0)
        return try_catch.SERVICE_TRY_RES(
          {
            resultado:
              "No se encontraron herramientas registradas en la base de datos",
          },
          404
        );
      ...
      return try_catch.SERVICE_CATCH_RES(
        err,
        "No se pueden ver las herramientas debido a una falla en el sistema"
      );
    }
  };
  ...
  crearHerramienta = async (data) => {
    try {
      ...
      if (!imageError) {
        return try_catch.SERVICE_TRY_RES(
          "La creación de la herramienta finalizó exitosmente",
          201
        );
      } else {
        return try_catch.SERVICE_TRY_RES(
          "La creación de la herramienta finalizó exitosamente, pero ocurrió un error al querer guardar la imagen",
          500
        );
      }
    } catch (err) {
      return try_catch.SERVICE_CATCH_RES(
        err,
        "La creación de la herramienta falló"
      );
    }
  };
  deshabilitarHerramienta = async (id_tool) => {
    try {
      ...
      return try_catch.SERVICE_TRY_RES(
        "La deshabilitación de la herramienta finalizó exitosamente",
        200
      );
    } catch (err) {
      return try_catch.SERVICE_CATCH_RES(
        err,
        "La deshabilitación de la herramienta falló"
      );
    }
  };
  borrarHerramienta = async (id_tool) => {
    try {
      ...
      return try_catch.SERVICE_TRY_RES(
        "La eliminación de la herramietna finalizó exitosamente",
        200
      );
    } catch (err) {
      return try_catch.SERVICE_CATCH_RES(
  ...
  updateTool = async (id_tool, data) => {
    try {
      ...
      if (!imageError) {
        return try_catch.SERVICE_TRY_RES(
          "La actualización de la herramienta finalizó exitosamente",
          200
        );
      } else {
        return try_catch.SERVICE_TRY_RES(
          "La actualización de la herramienta finalizó exitosamente, pero ocurrió un error al querer guardar la imagen",
          500
        );
      }
    } catch (err) {
      return try_catch.SERVICE_CATCH_RES(
        err,
        "La actualización de la herramienta falló"
      );
    }
  };
