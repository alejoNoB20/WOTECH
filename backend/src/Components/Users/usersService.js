  ...
  crearUser = async (user) => {
    try {
      ...
      return try_catch.SERVICE_CATCH_RES(
        err,
        "Error del servidor al momento de registrar el usuario"
      );
    }
  };
  ...
  loginUser = async (res, user) => {
    try {
      ...
      const verifyPassword = bcrypt.compareSync(
        password_user,
        response.password_user
      );
      if (!verifyPassword)
        return try_catch.SERVICE_TRY_RES("La contraseña es incorrecta", 400);
      ...
      return try_catch.SERVICE_CATCH_RES(
        err,
        "Error del servidor al momento de checkear el usuario"
      );
    }
  };
}
