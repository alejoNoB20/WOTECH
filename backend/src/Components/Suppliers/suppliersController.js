...
export class supplierController {
  ...
  verPaginas = async (req, res) => {
    try {
      const resultado = await Supplier.verPaginasProveedores(req.params.page);
      try_catch.TRY_RES(res, resultado);
    } catch (err) {
      try_catch.CATCH_RES(res, err);
    }
  ...
  deshabilitar = async (req, res) => {
    try {
      const resultado = await Supplier.deshabilitarProveedor(
  ...
  detalles = async (req, res) => {
    try {
      const resultado = await Supplier.filtrarProveedor(
        null,
        "id_supplier",
        req.params.id_supplier
      );
  ...
  actualizar = async (req, res) => {
    try {
      const resultado = await Supplier.actualizarProveedor(
        req.params.id_supplier,
        req.body
      );
      try_catch.TRY_RES(res, resultado);
