...
export const productValidator = {
    createTool:[
        body('name_product')
            .trim()
            .notEmpty({checkFalsy: true}).withMessage('El campo NOMBRE es obligatorio').bail()
            .isLength({max: 80}).withMessage('El campo NOMBRE permite un máximo de 80 caracteres').bail()
            .custom(async (value, {req}) => {
                const findSameName = await Product.llamarUnProducto(req.body.name_product);
        ...
        body('tools')
            ...
            .custom(value => {
                value.forEach(id_tool => {
                    if(isNaN(id_tool) || !Number.isInteger(id_tool) || id_tool < 0){
